...
- 从配置中移除的表整体删除；
- 只包含默认值的表，不会被写进本来没有它的文件。

合并结果在写盘前会再解析一次校验：若无法回环解析，则回退为整篇原子重写，绝不会写出损坏的配置。

> `eget config import` 是例外：它按设计整体替换目标文件，因此不保留源文件的注释与格式。
...
外部包不会写入安装记录。`eget list` 以 `manager:package` 展示，Source 列为管理器名；`eget ext list` 展示各管理器的可执行文件、可用性与包数量。
