...
func (s Service) resolveInstallOptionsWithConfig(cfg *cfgpkg.File, target string, source cfgpkg.Section, pkg cfgpkg.Section, cli install.Options, preferCacheDir bool) (install.Options, error) {
	...
	return install.Options{
		...
		// Silent and DeferInstaller are caller-only decisions: the config file has
		// no say, so they are copied straight from the caller and must not be lost
		// in this assembly.
		Silent:              cli.Silent,
		DeferInstaller:      cli.DeferInstaller,
